...
void UMDFastBindingGraphNode::AllocateDefaultPins()
{
	if (UMDFastBindingObject* Object = BindingObject.Get())
	{
		...
		for (FMDFastBindingItem& Item : Object->GetBindingItems())
		{
			...
			if (const FProperty* ItemProp = Item.ResolveOutputProperty())
			{
			...
			if (Pin != nullptr)
			{
				...
				else
				{
					...
					if (Pin->PinType.PinCategory == UEdGraphSchema_K2::PC_Struct && Pin->PinType.PinSubCategoryObject == ColorStruct)
					{
